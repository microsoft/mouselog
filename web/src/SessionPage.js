...
class SessionPage extends React.Component {
  ...
  getSessionCount() {
    WebsiteBackend.getWebsite(this.state.websiteId).then((res) => {
  ...
  deleteSession(i) {
    SessionBackend.deleteSession(
      this.state.sessions[i].id,
      this.state.websiteId
    )
      .then((res) => {
            ...
            <div>
              <Tooltip placement="topLeft" title="View">
                <Button
                  ...
                  onClick={() =>
                    Setting.openLink(
                      `/websites/${this.state.websiteId}/sessions/${record.id}/impressions`
                    )
                  }
                />
              </Tooltip>
        ...
        <Row type="flex" justify="end" style={{ marginRight: 20 }}>
          <Col>
            <Select
              ...
              onChange={(value) => {
                this.onPageSizeChange.call(this, value);
              }}
            >
  ...
  render() {
    return (
      <div>
        ...
        <Row>
          <Col span={24}>{this.renderTable(this.state.sessions)}</Col>
        </Row>
      </div>
    );
  }
